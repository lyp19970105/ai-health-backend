    ...
    public Flux<CommonChatResponse> sendMessageStream(String apiKey, String userInput, String conversationId) {
        ...
        try {
            ...
            return webClient.post()
                    ...
                    .mapNotNull(line -> {
                        // 3. 解析每一行 SSE 数据
                        log.trace("[Dify客户端] 收到原始数据: {}", line);
                        // SSE 规范中，以 "data:" 开头的行才是有效数据。
                        ...
                        try {
                            ...
                            DifySseEvent event = objectMapper.readValue(jsonData, DifySseEvent.class);
                            ...
                            log.warn("[Dify客户端] 解析Dify SSE事件失败: {}, 原始JSON: '{}'", e.getMessage(), jsonData);
                            return null;
                        }
                    })
